...
import {
    TOGGLE_SIDEBAR, TOGGLE_TOPBAR, ADD_TWEET, SET_TWEETS, REMOVE_TWEET, TOGGLE_STAR_TWEET,
    UPDATE_TWEET, TWEET_RETWEETED, toggleStarTweet, TWEET_UN_RETWEETED, TWEET_LIKED, TWEET_UN_LIKED
} from "../actions";
...
export function tweetsReducer(state:Array<Tweet> = [], action:Action):Array<Tweet> {
    ...
    switch (action.type) {
        ...
        case TOGGLE_STAR_TWEET:
            ({id} = action.payload);
            return state.map(tweet => tweet.id == id? tweetReducer(tweet, {type: action.type, payload: {tweet}}): tweet);
        default:
            return state;
    }
}
